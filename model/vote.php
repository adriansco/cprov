...
class vote
{
	...
	public function Listar($id)
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT
				...
				FROM
					poll_options P
				INNER JOIN polls C ON P.poll_id = C.id
				WHERE
			c.id = ?");
	...
	public function ListarPreguntas($propietario)
	{
		try
		{
			$result = array();
			...
			*
			...
			WHERE
				`owner` = ?
			AND `status` = ?");
			$stm->execute(array($propietario,1));
			return $stm->fetchAll(PDO::FETCH_OBJ);
		}
		catch(Exception $e)
		{
	...
	public function Actualizar(vote $data, array $field_values_array)
	{
		/* print '<pre>';
	...
	public function Registrar(vote $data, array $field_values_array)
	{
